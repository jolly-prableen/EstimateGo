...
type Bill = { id: string; customer: string; total: number; subtotal?: number; tax?: number; payment_status: string; payment_mode?: string; created_by?: string; created_at: string; items?: { name: string; quantity: number; price: number; productId?: string }[] };
...
export default function Index() {
  ...
  useEffect(() => {
    if (Platform.OS === "web") {
      try {
        const raw = (window as any).localStorage?.getItem("eg_session");
        if (raw) {
  ...
  const onLogin = (s: Session) => {
    AUTH = s.token;
    setSession(s);
    if (Platform.OS === "web") { try { (window as any).localStorage.setItem("eg_session", JSON.stringify(s)); } catch {} }
  };
  const signOut = () => {
    AUTH = "";
    if (Platform.OS === "web") { try { (window as any).localStorage.removeItem("eg_session"); } catch {} }
    setSession(null);
  };
...
function Workspace({ session, signOut }: { session: Session; signOut: () => void }) {
  ...
  const refresh = async () => {
    try {
      const requests: Promise<any>[] = [call("/dashboard"), call("/products"), call("/bills"), call("/customers"), call("/documents"), call("/profile")];
      if (role === "admin") requests.push(call("/users"));
      const [d, p, b, c, v, pr, u] = await Promise.all(requests);
      setDashboard(d); setProducts(p); setBills(b); setCustomers(c); setDocs(v); setProfile(pr);
      if (u) setUsers(u);
    } catch (e: any) {
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.content}>{tab === "Home" && <Home dashboard={dashboard} bills={bills} setTab={setTab} onSelect={(b: any) => { setSelectedBill(b); setEditingBill(null); setTab("Bills"); }} />}{tab === "Bills" && <Bills role={role} products={products} bills={bills} customers={customers} refresh={refresh} selected={selectedBill} setSelected={setSelectedBill} editingBill={editingBill} setEditingBill={setEditingBill} />}{tab === "Vault" && <Vault docs={docs} refresh={refresh} />}{tab === "Stock" && <Stock products={products} refresh={refresh} />}{tab === "Profile" && <Profile role={role} profile={profile} setProfile={setProfile} refresh={refresh} customers={customers} users={users} signOut={signOut} />}</View><View style={styles.tabs}>{([ ["Home", "grid-outline"], ["Bills", "receipt-outline"], ["Vault", "folder-open-outline"], ["Stock", "cube-outline"], ["Profile", "person-outline"] ] as [Tab, keyof typeof Ionicons.glyphMap][]).map(([name, icon]) => <Pressable testID={`tab-${name.toLowerCase()}`} key={name} onPress={() => setTab(name)} style={styles.tab}><Ionicons name={icon} size={22} color={tab === name ? C.green : C.muted} /><Text style={[styles.tabText, tab === name && { color: C.green, fontWeight: "700" }]}>{name}</Text></Pressable>)}</View></SafeAreaView>;
}

function Header({ title, subtitle }: { title: string; subtitle?: string }) { return <View style={styles.header}><View><Text style={styles.kicker}>EstimateGo</Text><Text style={styles.title}>{title}</Text>{subtitle && <Text style={styles.muted}>{subtitle}</Text>}</View><View style={styles.avatar}><Ionicons name="business-outline" size={20} color={C.green} /></View></View>; }
function Home({ dashboard, bills, setTab, onSelect }: any) { const hr = new Date().getHours(); const greet = hr < 12 ? "Good morning" : hr < 17 ? "Good afternoon" : hr < 21 ? "Good evening" : "Good night"; return <ScrollView contentContainerStyle={styles.scroll}><Header title={greet} subtitle="Your wholesale desk at a glance" /><View style={styles.hero}><Text style={styles.heroLabel}>TODAY'S SALES</Text><Text style={styles.heroAmount}>{money(dashboard?.total_sales || 0)}</Text><View style={styles.heroFoot}><Text style={styles.heroSub}>{dashboard?.bill_count || 0} bills today</Text><View style={styles.up}><Ionicons name="arrow-up" size={14} color={C.green} /><Text style={styles.upText}>Live</Text></View></View><Text style={{ color: "#C9D8C5", fontSize: 11, marginTop: 10 }}>All time: {money(dashboard?.lifetime_sales || 0)} · {dashboard?.lifetime_bill_count || 0} bills</Text></View><Text style={styles.section}>QUICK ACTIONS</Text><View style={styles.actionRow}><Action icon="add-circle-outline" label="New bill" onPress={() => setTab("Bills")} /><Action icon="cloud-upload-outline" label="Store doc" onPress={() => setTab("Vault")} /><Action icon="cube-outline" label="Add stock" onPress={() => setTab("Stock")} /></View><View style={styles.sectionRow}><Text style={styles.section}>RECENT BILLS</Text><Pressable onPress={() => setTab("Bills")}><Text style={styles.link}>View all</Text></Pressable></View>{bills.length ? bills.slice(0, 4).map((b: Bill) => <BillRow key={b.id} bill={b} onPress={() => onSelect(b)} />) : <Empty icon="receipt-outline" text="Your first bill will appear here" />}</ScrollView>; }
...
function Bills({ role, products, bills, customers, refresh, selected, setSelected, editingBill, setEditingBill }: any) {
  ...
  const addProduct = (p: Product) => setItems((old) => [...old, { productId: p.id, name: p.name, price: p.price, quantity: 1 }]);
  const subtotal = items.reduce((s, x) => s + x.price * x.quantity, 0);

  const resetForm = () => { setCustomer(""); setItems([]); setTax("0"); setPayMode("Cash"); setSaveCust(true); };
  const afterSave = (bill: any, wasEdit: boolean) => {
    Alert.alert(wasEdit ? "Bill updated" : "Bill saved automatically", `${bill.id} is stored in your bills.`, [{ text: "Share", onPress: () => shareEstimate(bill) }, { text: "Print", onPress: () => printEstimate(bill) }, { text: "Done", style: "cancel" }]);
  };
  const save = async () => {
    if (!customer.trim() || !items.length) return Alert.alert("Almost ready", "Add a customer and at least one product.");
    const body = JSON.stringify({ customer, items, tax_rate: Number(tax), payment_mode: payMode });
    try {
      let bill: any;
      if (isEditing) {
        bill = await call(`/bills/${editingBill.id}`, { method: "PUT", body });
        setEditingBill(null);
      } else {
        bill = await call("/bills", { method: "POST", body });
      ...
      Alert.alert(isEditing ? "Couldn't update bill" : "Couldn't save bill", e.status === 403 ? "Only admin can edit bills." : "Please try again.");
    }
  };
  ...
  return <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}><ScrollView contentContainerStyle={styles.scroll}><Header title={isEditing ? `Edit ${editingBill.id}` : "New bill"} subtitle={isEditing ? "Change items, then save" : "Build an estimate in a few taps"} />
    ...
    {suggestions.length > 0 && <View style={styles.sugWrap}>{suggestions.map((c: Customer) => <Pressable key={c.id} style={styles.sugRow} onPress={() => setCustomer(c.name)}><Text style={styles.rowTitle}>{c.name}</Text><Text style={styles.muted}>Pick</Text></Pressable>)}</View>}
    ...
    <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>{["Cash", "UPI", "Card", "Credit"].map((m) => <Pressable key={m} onPress={() => setPayMode(m)} style={[styles.filter, payMode === m && styles.filterActive]}><Text style={[styles.filterText, payMode === m && { color: C.white }]}>{m}</Text></Pressable>)}</View>
...
function BillDetail({ role, bill, onBack, onStatus, onDelete, onEdit }: any) {
...
function Profile({ role, profile, setProfile, refresh, customers, users, signOut }: any) {
  const isAdmin = role === "admin";
  ...
  const addUser = async () => { try { await call("/users", { method: "POST", body: JSON.stringify({ username: newUser, password: newPass, role: newRole }) }); setNewUser(""); setNewPass(""); refresh(); Alert.alert("Account created", `${newUser} can now sign in.`); } catch (e: any) { Alert.alert("Couldn't create account", e.status === 400 ? "Username taken or password too short (4+ chars)." : e.status === 403 ? "Admin access required." : "Please try again."); } };
